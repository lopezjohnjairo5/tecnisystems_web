<?php
	...
	if (isset($valid_sett_email) && isset($valid_sett_pass) && !empty($valid_sett_new_pass) && !empty($valid_sett_conf_pass))
  {
    if ($valid_sett_new_pass == $valid_sett_conf_pass && strlen($valid_sett_new_pass) >= 8)
    {
      ...
      if ($user_exist_in_bd)
      {
        $result_update = update_pass_technical_user($valid_sett_email,$encripted_new_pass,$_SESSION["technical_user"][0]);
        if ($result_update == true)
        {
          echo "Contraseña actualizada correctamente.";
          $message = "<p>Se ha realizado el cambio de la contraseña para el usuario: ".$valid_sett_email." en la plataforma reportes.</p><br><span>Si usted no solicito este cambio por favor comuniquese con nosotros.</span>";
